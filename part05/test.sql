...
isRequired: BIT -> BOOOLEAN because BOOLEAN is more intuitive; default to false.
dateAdded: changed DATETIME to TIMESTAMP because we we're running to issues.
msrp: added a check constraint on MSRP to prevent negative values
*/
...
/*  DONE
Explanation:
changed order of AppUser to be above other tables because order matters.
...
dateJoined: DATETIME to TIMESTAPM to work with CURRENT_TIMESTAMP
...
isActive: TINYINT(1) -> BOOLEAN: Using BOOLEAN type to increase readibilty, also to be able to use True and False.
datePosted: TIMESTAMP with default CURRENT_TIMESTAMP
added ON DELETE CASCADE to maintain data integrity.
...
/* DONE
Explanation:
email length changed to 100 to maintain consistency with other tables.
